import asyncio
from typing import Optional
from dataclasses import dataclass
import time
from order_executor import OrderExecutor
from config import MarketMakerConfig

@dataclass
class PendingOrder:
    order_id: str = ""
    side: str = ""
    price: float = 0.0
    quantity: float = 0.0
    placed_time: float = 0.0
    is_pending: bool = False

class MarketMaker:
    def __init__(self, orderbook, symbol):
        self.symbol = symbol
        self.orderbook = orderbook
        self.order_executor = OrderExecutor("mx0vglIZOoLYMTFKD6", "f319935c8e2242beb14854c3d12a849f")
        self.current_buy: Optional[PendingOrder] = None
        self.current_sell: Optional[PendingOrder] = None
        self.buy_lock = asyncio.Lock()
        self.sell_lock = asyncio.Lock()
        
        # Load config
        self.config = MarketMakerConfig()
        self.tick_size = self.config.TICK_SIZE
        self.order_quantity = self.config.ORDER_QUANTITY
        self.spread_ticks = self.config.SPREAD_TICKS
        self.aggressive_mode = self.config.AGGRESSIVE_MODE
        self.min_spread_percentage = self.config.MIN_SPREAD_PERCENTAGE
        self.max_position = self.config.MAX_POSITION
        self.position_skew_factor = self.config.POSITION_SKEW_FACTOR
        
        # Position tracking
        self.position = 0.0  # Current inventory position
        
        # P&L tracking
        self.total_buy_value = 0.0
        self.total_sell_value = 0.0
        self.total_buy_quantity = 0.0
        self.total_sell_quantity = 0.0
        self.realized_pnl = 0.0
        self.trades_count = 0
        
        # Trading state
        self.is_trading_enabled = True
        self.last_update_time = time.time()

    async def on_orderbook_update(self):
        if not self.is_trading_enabled:
            return
            
        if self.get_best_bid() == 0 or self.get_best_ask() == 0:
            return
        
        # Check stop loss
        if self.realized_pnl < self.config.STOP_LOSS_AMOUNT:
            print(f"STOP LOSS HIT: P&L {self.realized_pnl:.4f} < {self.config.STOP_LOSS_AMOUNT}")
            self.is_trading_enabled = False
            await self.cancel_all_orders()
            return
        
        # Check take profit
        if self.config.TAKE_PROFIT_AMOUNT > 0 and self.realized_pnl >= self.config.TAKE_PROFIT_AMOUNT:
            print(f"TAKE PROFIT HIT: P&L {self.realized_pnl:.4f} >= {self.config.TAKE_PROFIT_AMOUNT}")
            self.is_trading_enabled = False
            await self.cancel_all_orders()
            return
        
        # Check spread conditions
        spread_pct = self.orderbook.get_spread_percentage()
        if spread_pct > self.config.MAX_SPREAD_TO_TRADE:
            return  # Spread too wide, don't trade

        asyncio.create_task(self.handle_buy())
        asyncio.create_task(self.handle_sell())

    async def handle_buy(self):
        async with self.buy_lock:
            await self.check_current_buy_order()
            await self.place_new_buy_order_if_needed()

    async def handle_sell(self):
        async with self.sell_lock:
            await self.check_current_sell_order()
            await self.place_new_sell_order_if_needed()

    def get_best_bid(self):
        return self.orderbook.get_best_bid()

    def get_best_ask(self):
        return self.orderbook.get_best_ask()

    def is_only_one_at_current_bid_level(self):
        if not self.current_buy:
            return False
        return self.orderbook.get_bid_quantity(self.current_buy.price) == self.current_buy.quantity

    def is_only_one_at_current_ask_level(self):
        if not self.current_sell:
            return False
        return self.orderbook.get_ask_quantity(self.current_sell.price) == self.current_sell.quantity

    def should_cancel_current_buy(self):
        distance_to_second_bid = round(self.get_best_bid() - self.orderbook.get_second_best_bid(), 6)
        if self.is_only_one_at_current_bid_level() and distance_to_second_bid > self.tick_size:
            return True
        return self.current_buy.price != self.get_best_bid()

    def should_cancel_current_sell(self):
        distance_to_second_ask = round(self.orderbook.get_second_best_ask() - self.get_best_ask(), 6)
        if self.is_only_one_at_current_ask_level() and distance_to_second_ask > self.tick_size:
            return True
        return self.current_sell.price != self.get_best_ask()

    async def check_current_buy_order(self):
        if self.current_buy and self.current_buy.order_id and not self.current_buy.is_pending:
            # Check order age
            order_age = time.time() - self.current_buy.placed_time
            
            status = await self.order_executor.get_order_status(self.symbol, self.current_buy.order_id)
            if status.success:
                if status.status in ["FILLED", "CANCELED"]:
                    if status.status == "FILLED":
                        self.on_buy_filled(self.current_buy.price, self.current_buy.quantity)
                    print(f"Buy order {self.current_buy.order_id} is {status.status}")
                    self.current_buy = None
                elif self.should_cancel_current_buy() or order_age > self.config.MAX_ORDER_AGE_SECONDS:
                    reason = "stale" if order_age > self.config.MAX_ORDER_AGE_SECONDS else "price moved"
                    print(f"Cancelling buy order ({reason}): {self.current_buy.order_id}")
                    self.current_buy.is_pending = True  # Mark as pending BEFORE cancel
                    cancel_resp = await self.order_executor.cancel_order(self.symbol, self.current_buy.order_id)
                    if cancel_resp.success:
                        self.current_buy = None
                    else:
                        self.current_buy.is_pending = False  # Reset if cancel failed

    async def check_current_sell_order(self):
        if self.current_sell and self.current_sell.order_id and not self.current_sell.is_pending:
            # Check order age
            order_age = time.time() - self.current_sell.placed_time
            
            status = await self.order_executor.get_order_status(self.symbol, self.current_sell.order_id)
            if status.success:
                if status.status in ["FILLED", "CANCELED"]:
                    if status.status == "FILLED":
                        self.on_sell_filled(self.current_sell.price, self.current_sell.quantity)
                    print(f"Sell order {self.current_sell.order_id} is {status.status}")
                    self.current_sell = None
                elif self.should_cancel_current_sell() or order_age > self.config.MAX_ORDER_AGE_SECONDS:
                    reason = "stale" if order_age > self.config.MAX_ORDER_AGE_SECONDS else "price moved"
                    print(f"Cancelling sell order ({reason}): {self.current_sell.order_id}")
                    self.current_sell.is_pending = True  # Mark as pending BEFORE cancel
                    cancel_resp = await self.order_executor.cancel_order(self.symbol, self.current_sell.order_id)
                    if cancel_resp.success:
                        self.current_sell = None
                    else:
                        self.current_sell.is_pending = False  # Reset if cancel failed

    async def place_new_buy_order_if_needed(self):
        if not self.current_buy or (self.current_buy and not self.current_buy.order_id and not self.current_buy.is_pending):
            # Check position limits
            if self.position >= self.max_position:
                return  # Don't place buy orders if at max long position
            
            buy_price = self.calculate_buy_price()

            self.current_buy = PendingOrder(
                side="BUY",
                price=buy_price,
                quantity=self.order_quantity,
                placed_time=time.time(),
                is_pending=True
            )

            resp = await self.order_executor.place_buy_limit_order(self.symbol, buy_price, self.order_quantity)

            if resp.success:
                self.current_buy.order_id = resp.order_id
                self.current_buy.is_pending = False  # No longer pending
                print(f"Placed buy order: {resp.order_id} @ {buy_price}")
            else:
                print(f"Failed to place buy order: {resp.error_message}")
                self.current_buy = None  # Reset on failure

    async def place_new_sell_order_if_needed(self):
        if not self.current_sell or (
                self.current_sell and not self.current_sell.order_id and not self.current_sell.is_pending):
            # Check position limits
            if self.position <= -self.max_position:
                return  # Don't place sell orders if at max short position
            
            sell_price = self.calculate_sell_price()

            self.current_sell = PendingOrder(
                side="SELL",
                price=sell_price,
                quantity=self.order_quantity,
                placed_time=time.time(),
                is_pending=True
            )

            resp = await self.order_executor.place_sell_limit_order(self.symbol, sell_price, self.order_quantity)

            if resp.success:
                self.current_sell.order_id = resp.order_id
                self.current_sell.is_pending = False  # No longer pending
                print(f"Placed sell order: {resp.order_id} @ {sell_price}")
            else:
                print(f"Failed to place sell order: {resp.error_message}")
                self.current_sell = None  # Reset on failure
    
    def calculate_buy_price(self) -> float:
        """Calculate optimal buy price based on spread strategy and position"""
        best_bid = self.get_best_bid()
        best_ask = self.get_best_ask()
        
        if best_bid == 0 or best_ask == 0:
            return 0
        
        spread = best_ask - best_bid
        min_spread = best_bid * self.min_spread_percentage / 100
        
        # If spread is too tight, join the best bid
        if spread <= min_spread:
            return best_bid
        
        # Position-based price adjustment
        position_adjustment = 0
        if self.position > 0:  # Long position, be less aggressive buying
            position_adjustment = -self.tick_size * self.position_skew_factor * (self.position / self.max_position)
        elif self.position < 0:  # Short position, be more aggressive buying
            position_adjustment = self.tick_size * self.position_skew_factor * abs(self.position / self.max_position)
        
        if self.aggressive_mode:
            # Join or improve best bid
            buy_price = best_bid + self.tick_size
        else:
            # Use spread-based pricing
            buy_price = best_bid - (self.tick_size * self.spread_ticks)
        
        # Apply position adjustment
        buy_price = round(buy_price + position_adjustment, 6)
        
        # Ensure we don't cross the spread
        if buy_price >= best_ask:
            buy_price = best_ask - self.tick_size
        
        return max(buy_price, best_bid - (self.tick_size * 10))  # Don't go too far from best bid
    
    def calculate_sell_price(self) -> float:
        """Calculate optimal sell price based on spread strategy and position"""
        best_bid = self.get_best_bid()
        best_ask = self.get_best_ask()
        
        if best_bid == 0 or best_ask == 0:
            return 0
        
        spread = best_ask - best_bid
        min_spread = best_ask * self.min_spread_percentage / 100
        
        # If spread is too tight, join the best ask
        if spread <= min_spread:
            return best_ask
        
        # Position-based price adjustment
        position_adjustment = 0
        if self.position > 0:  # Long position, be more aggressive selling
            position_adjustment = -self.tick_size * self.position_skew_factor * (self.position / self.max_position)
        elif self.position < 0:  # Short position, be less aggressive selling
            position_adjustment = self.tick_size * self.position_skew_factor * abs(self.position / self.max_position)
        
        if self.aggressive_mode:
            # Join or improve best ask
            sell_price = best_ask - self.tick_size
        else:
            # Use spread-based pricing
            sell_price = best_ask + (self.tick_size * self.spread_ticks)
        
        # Apply position adjustment
        sell_price = round(sell_price + position_adjustment, 6)
        
        # Ensure we don't cross the spread
        if sell_price <= best_bid:
            sell_price = best_bid + self.tick_size
        
        return min(sell_price, best_ask + (self.tick_size * 10))  # Don't go too far from best ask
    
    def on_buy_filled(self, price: float, quantity: float):
        """Update position and P&L when buy order is filled"""
        self.position += quantity
        self.total_buy_value += price * quantity
        self.total_buy_quantity += quantity
        self.trades_count += 1
        
        print(f"Buy filled: {quantity} @ {price} | Position: {self.position:.2f} | Avg Buy: {self.get_avg_buy_price():.6f}")
        self.print_pnl_status()
    
    def on_sell_filled(self, price: float, quantity: float):
        """Update position and P&L when sell order is filled"""
        self.position -= quantity
        self.total_sell_value += price * quantity
        self.total_sell_quantity += quantity
        self.trades_count += 1
        
        # Calculate realized P&L for this trade
        if self.total_buy_quantity > 0:
            avg_buy = self.get_avg_buy_price()
            trade_pnl = (price - avg_buy) * quantity
            self.realized_pnl += trade_pnl
        
        print(f"Sell filled: {quantity} @ {price} | Position: {self.position:.2f} | Avg Sell: {self.get_avg_sell_price():.6f}")
        self.print_pnl_status()
    
    def get_avg_buy_price(self) -> float:
        """Get average buy price"""
        if self.total_buy_quantity == 0:
            return 0
        return self.total_buy_value / self.total_buy_quantity
    
    def get_avg_sell_price(self) -> float:
        """Get average sell price"""
        if self.total_sell_quantity == 0:
            return 0
        return self.total_sell_value / self.total_sell_quantity
    
    def get_unrealized_pnl(self) -> float:
        """Calculate unrealized P&L based on current position and mid price"""
        if self.position == 0:
            return 0
        
        best_bid = self.get_best_bid()
        best_ask = self.get_best_ask()
        
        if best_bid == 0 or best_ask == 0:
            return 0
        
        mid_price = (best_bid + best_ask) / 2
        avg_buy = self.get_avg_buy_price()
        
        if avg_buy == 0:
            return 0
        
        return (mid_price - avg_buy) * self.position
    
    def print_pnl_status(self):
        """Print current P&L status"""
        unrealized = self.get_unrealized_pnl()
        total_pnl = self.realized_pnl + unrealized
        
        spread = self.orderbook.get_spread()
        spread_pct = self.orderbook.get_spread_percentage()
        
        print(f"=== P&L Status ===")
        print(f"Trades: {self.trades_count} | Spread: {spread:.6f} ({spread_pct:.3f}%)")
        print(f"Realized P&L: {self.realized_pnl:.4f} | Unrealized: {unrealized:.4f} | Total: {total_pnl:.4f}")
        print(f"===================")
    
    async def cancel_all_orders(self):
        """Cancel all pending orders"""
        tasks = []
        
        if self.current_buy and self.current_buy.order_id:
            print(f"Cancelling buy order: {self.current_buy.order_id}")
            tasks.append(self.order_executor.cancel_order(self.symbol, self.current_buy.order_id))
        
        if self.current_sell and self.current_sell.order_id:
            print(f"Cancelling sell order: {self.current_sell.order_id}")
            tasks.append(self.order_executor.cancel_order(self.symbol, self.current_sell.order_id))
        
        if tasks:
            results = await asyncio.gather(*tasks, return_exceptions=True)
            self.current_buy = None
            self.current_sell = None
            print(f"Cancelled {len(tasks)} orders")
    
    def get_status_summary(self) -> str:
        """Get a summary of the current market maker status"""
        return (f"Trading: {self.is_trading_enabled} | "
                f"Position: {self.position:.2f}/{self.max_position} | "
                f"P&L: {self.realized_pnl:.4f} | "
                f"Trades: {self.trades_count}")
